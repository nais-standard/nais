...
- **DNS discovery** — `_agent` TXT records pointing to the card and publishing the signing key (`k=`)
- **Signed agent cards** — `/.well-known/agent.json` carrying tags, auth, and payment info, sealed by a mandatory Ed25519 signature
...
Every card is signed with a detached EdDSA (Ed25519) JWS, and its `kid` must match the signing-key fingerprint published in DNS (`k=`). A web-server compromise alone cannot forge a card or swap the `payTo` address — an attacker would also need the DNS zone and the private signing key.
